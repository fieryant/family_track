...
const TOOLS = [
  {
    ...
    function: {
      name: "search_items",
      description:
        "Search the item catalog by name. Supports English and Bengali. Use this to find an item before adding it.",
      parameters: {
        type: "object",
        properties: {
          query: {
            type: "string",
            description: "Search term in English or Bengali",
          },
        },
        required: ["query"],
      },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Get all items currently in the shopping list for this session.",
  ...
  },
  {
    ...
    function: {
      name: "add_item",
      description:
        "Add an item to the shopping list. The item_name must be a catalog item name (search first if unsure).",
      parameters: {
        type: "object",
        properties: {
          ...
          unit_symbol: {
            type: "string",
            description: "Unit symbol like kg, g, pcs, L, ml",
          },
        },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Mark an item as bought, partial, removed, or pending. Use item name to identify it.",
      parameters: {
        type: "object",
        properties: {
          ...
          status: {
            type: "string",
            enum: ["bought", "partial", "removed", "pending"],
          },
          bought_amount: {
            type: "number",
            description: "How much was bought (for bought/partial)",
          },
          price: {
            type: "number",
  ...
  },
  {
    ...
    function: {
      name: "get_pantry",
      description:
        "Get the current pantry inventory — what is stocked at home.",
      parameters: { type: "object", properties: {} },
    },
  },
  {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Get a summary of the current shopping session: how many items are pending, bought, etc.",
      parameters: { type: "object", properties: {} },
    },
  },
];
...
async function handleSearchItems(ctx: ToolContext, args: { query: string }) {
  ...
  const { data: items } = await ctx.supabase
    .from("items")
    .select("id, name, category_id, unit_type_id")
    .in("id", allIds)
    .eq("is_active", true);
...
async function handleListShoppingItems(ctx: ToolContext) {
  ...
  for (const row of data ?? []) {
    summary[row.status as keyof typeof summary] =
      (summary[row.status as keyof typeof summary] ?? 0) + 1;
  }
...
async function handleAddItem(
  ...
  if (!itemId) {
    return {
      success: false,
      error: `Item "${args.item_name}" not found in catalog. Try searching first.`,
    };
  }
  ...
  if (!unitId) {
    return {
      success: false,
      error: `Unit "${args.unit_symbol}" not found. Try: kg, g, pcs, L, ml`,
    };
  }
...
async function handleUpdateItemStatus(
  ...
  const itemId = await resolveItemByName(ctx, args.item_name);
  if (!itemId)
    return { success: false, error: `Item "${args.item_name}" not found` };
  ...
  if (!row)
    return {
      success: false,
      error: `"${args.item_name}" is not in the current shopping list`,
    };
...
async function handleSetPantryAmount(
  ...
  if (!unitId)
    return {
      success: false,
      error: `Unit "${args.unit_symbol}" not recognized`,
    };
  ...
  const { error } = await ctx.supabase.from("pantry_items").upsert(
    {
      list_id: ctx.listId,
      item_id: itemId,
      amount: args.amount,
...
async function executeTool(
  ...
  switch (name) {
    case "search_items":
      return handleSearchItems(ctx, args as any);
    case "list_shopping_items":
      return handleListShoppingItems(ctx);
    case "add_item":
      return handleAddItem(ctx, args as any);
    case "update_item_status":
      return handleUpdateItemStatus(ctx, args as any);
    case "remove_item":
      return handleRemoveItem(ctx, args as any);
    case "get_pantry":
      return handleGetPantry(ctx);
    case "set_pantry_amount":
      return handleSetPantryAmount(ctx, args as any);
    case "get_session_summary":
      return handleGetSessionSummary(ctx);
    default:
